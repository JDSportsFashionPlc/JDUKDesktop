	...
	// -- use the #EDIT to modify anything else on this page
			...
			// MAKE THE CAROSEL
				...
				// BUILD THE THREE BOXES
					...
					var FUL = '';jQuery.each(x.Page[0].FirstUL, function(key, val){FUL += '<li>';FUL += '<a class="caps" href="'+val.link+'" manual_cm_re="Content-_-Links-_-CTRWomenPage-'+val.name+'">'+val.name+'</a>';FUL += '</li>';});FUL += '';jQuery('.FirstUL ul').append($(FUL));	
					jQuery('.SecondBox').css({background: 'url("'+x.Page[0].SecondBox+'") ', backgroundPosition: '-179px 0'});
					
					jQuery('.SecondBoxTitle').html(x.Page[0].SecondBoxTitle);
					var SUL = '';jQuery.each(x.Page[0].SecondUL, function(key, val){SUL += '<li>';SUL += '<a class="caps" href="'+val.link+'" manual_cm_re="Content-_-Links-_-CTRWomenPage-'+val.name+'">'+val.name+'</a>';SUL += '</li>';});SUL += '';jQuery('.SecondUL ul').append($(SUL));
					jQuery('.ThirdBox').css('background-image', 'url("'+x.Page[0].ThirdBox+'")');
					jQuery('.ThirdBoxTitle').html(x.Page[0].ThirdBoxTitle);
					var TUL = '';jQuery.each(x.Page[0].ThirdUL, function(key, val){TUL += '<li>';TUL += '<a class="caps" href="'+val.link+'" manual_cm_re="Content-_-Links-_-CTRWomenPage-'+val.name+'">'+val.name+'</a>';TUL += '</li>';});TUL += '';jQuery('.ThirdUL ul').append($(TUL));
