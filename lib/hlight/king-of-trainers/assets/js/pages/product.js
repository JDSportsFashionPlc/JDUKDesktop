...
// -- Load Program
	...
	jQuery(document).ready(function ($) {
		...
		jQuery.getJSON(""+path+"assets/js/pages/products/product.json", function (x) {	
			...
			// NOW WE RUN VARIOUS HEADER SPECIFIC CALLS FROM JSON
				...
				if(pageType=="Archive"){ // #Archive
					...
					jQuery('.kotProductSideShots').append('<div class="sideShotLeft kotProductSideShotsdiv left"><img src="'+x.Page[0].sideShotLeft+'"></div>');
					jQuery('.kotProductSideShots').append('<div class="sideShotRight kotProductSideShotsdiv left"><img src="'+x.Page[0].sideShotRight+'"></div>');
				}
		...
		*/
		...
		jQuery.getJSON(""+path+"assets/js/pages/latest.json", function (x) {	
			...
			galHTML+='<h3 class="gotham ultra">LATEST TRAINERS</h3>';
			galHTML+='<div class="ruhs">';
